...
require '../mailFunctionHelper.php';
...
if( $_SERVER["REQUEST_METHOD"] == "GET" && getMailVerificationValidation($_GET) ){
	...
	$search = "select * from contact_us where vid = '$v'";
    ...
    if(mysqli_num_rows($res) > 0){
    	...
    	if($row["status"] == "NO"){
    		...
    		$update = "update contact_us set update_datetime = '$datetime',update_user_agent = '$user_agent', update_user_ip = '$ip',status = 'YES' where vid = '$v' ";
    	...
    	echo "<h1 style='text-align:center;color:red'>Invalid User..</h1>";
    }
	...
	$datetime   = date("Y-m-d H:i:s");
 	...
 	$insertQuery = "Insert into contact_us(vid,name,mobile,email,message,datetime,user_agent,user_ip,status) ".
