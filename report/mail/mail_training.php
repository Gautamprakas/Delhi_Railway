...
require '../mailFunctionHelperTraining.php';
...
if( $_SERVER["REQUEST_METHOD"] == "GET" && getMailVerificationValidation($_GET) ){
	...
	$search = "select * from training_reg where vid = '$v'";
    ...
    if(mysqli_num_rows($res) > 0){
    	...
    	if($row["status"] == "NO"){
    		...
    		$update = "update training_reg set update_datetime = '$datetime',update_user_agent = '$user_agent', update_user_ip = '$ip',status = 'YES' where vid = '$v' ";
    	    mysqli_query($con,$update);
    		sendMailToAdmin( $adminMail , $adminPwd , $row["name"] , $row["mobile"] , $row["email"] , $row["message"] );
    	}
    	echo "<h1 style='text-align:center;color:green'>ThankYou for registration. We will contact you soon.</h1>";
	...
	$datetime   = date("Y-m-d H:i:s");
 	...
 	$insertQuery = "Insert into training_reg(vid,name,mobile,email,message,datetime,user_agent,user_ip,status) ".
