<!-- Jquery Core Js -->
    ...
    <script type="text/javascript">
        function submitData(){
                ...
                len =parseInt(len);
                for (var i = 1; i <= len; i++) {
                    var id ="#coachStatus"+i.toString();
                    var selectValueTag=$(id);
                    var selectValue=selectValueTag.val();
                    if (selectValue){
                        coachNo=selectValueTag.data('properties');
                        status=selectValue;
                        console.log(coachNo);
                        console.log(status);
                    }
                ...
                $.ajax({
                    url:'<?php echo base_url("view/CreateForm/saveUpdateCoach"); ?>',
                    method:'POST',
                    data:{
                        coachNo:coachNo,
                        status:status,
                    ...
                    success:function(response){
                        alert(response);
                        window.location.href='<?php echo base_url("view/CreateForm/editCoach"); ?>';
                    },
