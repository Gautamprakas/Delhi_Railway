...
class Api extends CI_Controller{
	...
	public function updateformschema(){
		...
		$form_data = update_form_ui($this,$req_id,$form_id,$family_id,$member_id);
		echo json_encode($form_data);
	}
	...
	public function railwayAppDropDown(){
        ...
        $none = count($train_numbers) > 0 ? $train_numbers[0] : "NONE";
        //END//

		$res = $this->db->get("railway_app_dropdown");

		foreach($res->result() as $index => $row){
			$data[$index] = $row;
			if( array_search($row->train_number, $train_numbers) === FALSE ){
				$data[$index]->train_number = $none;
			}
		}
